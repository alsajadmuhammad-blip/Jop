  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        ref={ref}
        className={cn(
          "fixed left-[50%] top-[50%] z-50 grid w-full max-w-[95vw] sm:max-w-lg max-h-[95vh] translate-x-[-50%] translate-y-[-50%] overflow-hidden overflow-y-auto gap-4 border bg-background p-4 sm:p-6 shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%] sm:rounded-lg",
          className
        )}
        {...props}
        // Conditionally render as a child or a div to avoid extra wrappers when using framer-motion
        asChild={asChild}
      >
        {/* Render children directly if asChild is true, otherwise wrap them for default styling */}
        {asChild ? (
          children
        ) : (
          <>
