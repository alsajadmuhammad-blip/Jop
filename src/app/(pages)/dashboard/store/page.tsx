...
import { PlusCircle, AlertTriangle, Edit, Trash2, Package, Image as ImageIcon, Package2, LogOut, Info, ShoppingCart as ShoppingCartIcon, Truck, Globe, CreditCard, CalendarDays, CheckCircle2, Clock, ExternalLink, Tag, Link2, Copy, Check } from "lucide-react";
...
function StoreSettingsTab({ store, onSettingChange, onLogoSave }: {
    ...
    const [slugDraft, setSlugDraft] = useState<string>(store.slug ?? slugify(store.name));
    ...
    const handleSlugChange = (v: string) => {
        ...
        if (cleaned.length < 3) setSlugError('الرابط قصير جداً (3 أحرف على الأقل)');
        else setSlugError('');
    };

    const handleSlugSave = async () => {
        if (slugError || slugDraft.length < 3) return;
        setSlugSaving(true);
        try {
            await onSettingChange('slug', slugDraft);
        } finally {
            setSlugSaving(false);
        }
    };
        ...
        <div className="space-y-4">
            ...
            <div className="rounded-2xl border border-primary/30 bg-white shadow-sm overflow-hidden">
                <div className="px-5 py-4 border-b border-slate-100 flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="text-base font-bold text-slate-900">رابط المتجر</h3>
                        <p className="text-xs text-slate-500 mt-0.5">رابط مخصص يمكنك مشاركته مع العملاء مباشرة.</p>
                    </div>
                </div>
                <div className="p-5 space-y-4">
                    {/* معاينة الرابط */}
                    <div className="flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
                        <span className="text-sm text-slate-400 truncate flex-1 font-mono" dir="ltr">{shareUrl}</span>
                        <button
                            onClick={handleCopy}
                            className="shrink-0 flex items-center gap-1.5 rounded-lg bg-white border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-100 transition-colors"
                        >
                            {copied
                                ? <><Check className="h-3.5 w-3.5 text-emerald-500" />تم النسخ</>
                    ...
                    <div className="space-y-2">
                        <Label className="text-sm font-semibold text-slate-700">تخصيص الرابط</Label>
                        <div className="flex gap-2">
                            <div className="flex flex-1 items-center rounded-xl border border-slate-200 bg-white overflow-hidden focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/10">
                                <span className="px-3 text-xs text-slate-400 whitespace-nowrap border-r border-slate-200 bg-slate-50 py-2.5 select-none">/s/</span>
                                <input
                                    type="text"
                                    value={slugDraft}
                            ...
                            <Button
                                ...
                                disabled={slugSaving || !!slugError || slugDraft.length < 3}
                                size="sm"
                                className="shrink-0 rounded-xl px-4 font-semibold"
                            >
                                {slugSaving ? 'جاري الحفظ…' : 'حفظ'}
                            </Button>
                        </div>
                        {slugError && <p className="text-xs text-red-500">{slugError}</p>}
                        <p className="text-xs text-slate-400">يمكن استخدام الأحرف الإنجليزية والعربية والأرقام والشرطة (-)</p>
                    </div>

                    {/* مشاركة سريعة */}
                    <a
                        href={`https://wa.me/?text=${encodeURIComponent(`تفضل بزيارة متجرنا على الرابط: ${shareUrl}`)}`}
                        target="_blank"
                    ...
                    >
                        <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.124.558 4.118 1.535 5.845L.057 23.428a.5.5 0 0 0 .609.61l5.647-1.48A11.953 11.953 0 0 0 12 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 22c-1.886 0-3.655-.515-5.173-1.41l-.372-.217-3.752.983.998-3.67-.239-.386A9.955 9.955 0 0 1 2 12C2 6.477 6.477 2 12 2s10 4.477 10 10-4.477 10-10 10z"/></svg>
                        مشاركة عبر واتساب
                    </a>
                </div>
            </div>
...
export default function StoreDashboardPage() {
  ...
  const updateStore = async (storeId: string, data: Partial<Store>) => {
    ...
    if (data.slug !== undefined) {
      dbData.slug = data.slug ? data.slug.toLowerCase().trim() : null;
    }
